...
public class CaesarCipher {
  public static void main (String[] args) throws Exception {
    ...
    if (args.length != 2) { 
      // When the number of parameters on the command-line is not 2, indicate the error
  ...
  public static String decode (String message, int key) {
    ...
    for (int index = 0; index < messageArr.length; index++) {
      ...
      // Subtracts the key from each character to obtain the new characters 
  ...
  public static int decryptCharCode (int charCode, int key) {
    // Rebase the original character code to 0-25
    int rebasedCharCode = charCode - 97;
    // Subtract the key's character code from the rebased message character 
    // code 
